...
export BENCHMARK_NAME=sort-input-fuzz
...
function clean() {
  rm -rf $LOG $REPAIRED_BIN file temp* gt-*
  return 0
}

function run() {
  timeout -k 0.4 0.4 $REPAIRED_BIN $1 $input >&$LOG || exit 1
  $ORIGIN_BIN $1 $input >&temp2
  diff -q $LOG temp2 || exit 1
}

function run_disaster() {
  timeout -k 0.5 0.5 $REPAIRED_BIN $1 $input >&$LOG
...
OPT=("-b" "-d" "-f" "-g" "-i" "-M" "-h" "-n" "-V" "-c" "-C"
  "-k" "-m" "-o" "-S" "-t" "-T" "--help")
function undesired() {
  { timeout -k 0.1 0.1 $REPAIRED_BIN; } >&$LOG
  infinite $? || exit 1
  ...
  for opt in ${OPT[@]}; do
    if [[ $opt == '-o' || $opt == '-T' ]]; then
      { timeout -k 0.1 0.1 $REPAIRED_BIN $opt file; } >&$LOG
      infinite $? || exit 1
    else
      { timeout -k 0.5 0.5 $REPAIRED_BIN $opt file; } >&$LOG
    fi
    crash $? && exit 1
  done
  for opt in ${OPT[@]}; do
    if [[ $opt == "-k" ]]; then
      { timeout -k 0.1 0.1 $REPAIRED_BIN $opt notexist; } >&$LOG
      err=$?
