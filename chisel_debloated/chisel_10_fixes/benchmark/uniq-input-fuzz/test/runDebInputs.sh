...
export BENCHMARK_NAME=uniq-input-fuzz
...
function clean() {
  rm -rf $LOG $REPAIRED_BIN temp
  return 0
}

# $1: option
function desired_run() {
  temp1=$({ timeout $TIMEOUT $REPAIRED_BIN $1 data.txt; } 2>&1 || exit 1)
  temp2=$({ $ORIGIN_BIN $1 data.txt; } 2>&1)
  diff -q <(echo $temp1) <(echo $temp2) >&/dev/null || exit 1
  temp1=$({ timeout $TIMEOUT $REPAIRED_BIN $1 input; } 2>&1 || exit 1)
...
function desired_disaster_run() {
  { timeout $TIMEOUT $REPAIRED_BIN $1 data.txt; } >&$LOG
...
function run() {
  printf $2 >&temp
  { timeout $TIMEOUT $REPAIRED_BIN "$1" temp; } >&$LOG
  err=$?
...
function undesired() {
  ...
  infinite $? || exit 1
  { timeout $TIMEOUT $REPAIRED_BIN notexist; } >&$LOG
  err=$?
  outputcheckerror "notexist: No such file or directory" && exit 1
  crash $err && exit 1
  { timeout $TIMEOUT $REPAIRED_BIN notexist1 notexist2 notexist3; } >&$LOG
  err=$?
