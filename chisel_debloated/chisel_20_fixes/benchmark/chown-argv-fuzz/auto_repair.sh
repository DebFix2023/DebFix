...
bash $compile_script src/chown.debloated.c $DEB_BIN
bash $compile_script src/chown.original.c  $ORIG_BIN 
...
do
    ...
    cd $cur_dir
    bash -x $TOOL_DIR/run.sh $cur_dir/src/chown.original.c $cur_dir/src/chown.debloated.c $crash_dir $crash_dir/reproduce.sh $cur_dir/test/runDebInputs.sh $cur_dir/compile.sh 20
    cd $cur_dir
done
