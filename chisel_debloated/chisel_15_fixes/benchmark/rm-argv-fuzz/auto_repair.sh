...
do
    ...
    do
        ...
        cd $cur_dir
        bash -x $TOOL_DIR/run.sh $cur_dir/src/rm.original.c $cur_dir/src/rm.debloated.c $crash_dir $crash_dir/reproduce.sh $cur_dir/test/runDebInputs.sh $cur_dir/compile.sh 15
        cd $cur_dir
    done
done