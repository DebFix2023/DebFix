...
bash $compile_script src/tar.debloated.c $DEB_BIN
bash $compile_script src/tar.original.c  $ORIG_BIN 
...
group_end_num=$(( $group_end_num<22?$group_end_num:22 ))
...
do
    ...
    do
        ...
        cd $cur_dir
        bash -x $TOOL_DIR/run.sh $cur_dir/src/tar.original.c $cur_dir/src/tar.debloated.c $crash_dir $crash_dir/reproduce.sh $cur_dir/test/runDebInputs.sh $cur_dir/compile.sh 15
        cd $cur_dir
    done
done
