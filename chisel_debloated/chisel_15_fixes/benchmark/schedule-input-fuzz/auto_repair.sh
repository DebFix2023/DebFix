...
bash $compile_script src/schedule.debloated.c $DEB_BIN
bash $compile_script src/schedule.original.c  $ORIG_BIN 
...
group_end_num=$(( $group_end_num<9?$group_end_num:9 ))
...
do
    ...
    do
        ...
        cd $cur_dir
        bash -x $TOOL_DIR/run.sh $cur_dir/src/schedule.original.c $cur_dir/src/schedule.debloated.c $crash_dir $crash_dir/reproduce.sh $cur_dir/test/runDebInputs.sh $cur_dir/compile.sh
        cd $cur_dir
    done
done