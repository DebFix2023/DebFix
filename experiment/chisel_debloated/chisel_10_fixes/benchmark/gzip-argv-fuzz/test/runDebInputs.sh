...
export BENCHMARK_NAME=gzip-argv-fuzz
...
export REPAIRED_BIN=$BENCHMARK_DIR/bin/gzip-1.2.4.reduced
...
function clean() {
  rm -rf $LOG $REPAIRED_BIN *.gz *.rb2 *.tst log* foo* bar* failures_compile failures_core
  return 0
}
...
function desired() {
  ...
  { timeout $TIMEOUT $REPAIRED_BIN -c <references/sample1.ref >sample1.rb2; } 2>$LOG
  crash $? && exit 1
  cmp sample1.gz sample1.rb2 >&/dev/null || exit 1
  # -d
  { timeout $TIMEOUT $REPAIRED_BIN -d <sample1.gz >sample1.tst; } 2>$LOG
  crash $? && exit 1
  cmp sample1.tst references/sample1.ref >&/dev/null || exit 1
  # -f
  echo "1234" >foo
  $ORIGIN_BIN -c foo >&/dev/null
  { timeout $TIMEOUT $REPAIRED_BIN -f foo; } 2>$LOG
  crash $? && exit 1
  # -t (expected to return 1)
  echo "1234" >foo
  { timeout $TIMEOUT $REPAIRED_BIN -t foo; } 2>$LOG
  if [[ $? -ne 1 ]]; then
    exit 1
  fi
  # -t
  { timeout $TIMEOUT $REPAIRED_BIN -t sample1.gz; } 2>$LOG
  crash $? && exit 1
  return 0
}
...
function undesired() {
  ...
  for opt in ${OPT1[@]}; do
    { timeout $TIMEOUT $REPAIRED_BIN $opt; } &>$LOG
  ...
  for opt in ${OPT0[@]}; do
    { timeout $TIMEOUT $REPAIRED_BIN -d $opt <foo.gz >sample1.tst; } 2>$LOG
  ...
  for opt in ${OPT3[@]}; do
    { timeout $TIMEOUT $REPAIRED_BIN -d $opt <bar.gz >sample1.tst; } 2>$LOG
    { timeout $TIMEOUT $REPAIRED_BIN -c $opt <references/sample1.ref >sample1.rb2; } 2>$LOG
    crash $? && exit 1
  done
  return 0
