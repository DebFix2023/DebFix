...
bash $compile_script src/uniq.debloated.c $DEB_BIN
bash $compile_script src/uniq.original.c  $ORIG_BIN 
...
do
    ...
    cd $cur_dir
    bash -x $TOOL_DIR/run.sh $cur_dir/src/uniq.original.c $cur_dir/src/uniq.debloated.c $crash_dir $crash_dir/reproduce.sh $cur_dir/test/runDebInputs.sh $cur_dir/compile.sh
    cd $cur_dir
done
